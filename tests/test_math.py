...
def test_vec3_distance_to_avoids_temporary_allocation():
    ...
    assert abs(v2.distance_to(v1) - 5.0) < 1e-6
    
    assert abs(v1.distance_to(v1)) < 1e-6


def test_vec3_distance_sq_to_matches_distance_to():
    
    v1 = Vec3(1.0, 2.0, 3.0)
    v2 = Vec3(4.0, 6.0, 3.0)
    assert abs(v1.distance_sq_to(v2) - 25.0) < 1e-6
    assert abs(v1.distance_sq_to(v2) - v1.distance_to(v2) ** 2) < 1e-6
